...
| 5 | Laser Hell | ❓ TBD | 🔴 Not discussed |
...
**Boundary — TWO ZONES:**
- **Outer Zone:** Full large square (same size as Dodge Hell) — this is where arrows spawn from the edges
- **Inner Zone:** Small box at screen center — heart is LOCKED here and cannot leave. Player only controls shield direction.
...
| 🟡 Yellow Arrow | Deceptive. Faces the OPPOSITE direction of travel (looks like it's going away). Spawns from one side but mid-flight snaps to attack from the OPPOSITE side. Player must react to the fake-out and flip shield. |
...
**Special Rules:**
- Shield Hell is a **pure penalty phase** — you cannot gain time here under any circumstance
...
| Climbing Sub-Phase | 8–9 strings | See special mechanics below |

**Movement:** Free X-axis movement. Y-axis locked — Up/Down instantly snaps heart to the adjacent string.

**Projectile Types**

| Projectile | Behavior |
|------------|----------|
| 🕷️ Spider | Travels horizontally across a specific string lane. Snap to a different string to dodge. |
| 🎱 Bouncing Ball | Moves diagonally, bounces off top and bottom boundary walls. Crosses multiple lanes — timing-based. |
| 🪃 Boomerang | Travels along one string to the far wall, reverses, returns on the same string. Must be dodged twice. |
| 💣 Large Bomb | 5-string phase only. Covers 3 adjacent strings. One at a time. Long visible countdown before detonation. Player must vacate all 3 covered strings before it blows. |
...
**Special Mechanics — Climbing Sub-Phase:**
...
- Monster sits stationary at the bottom with a **pulling animation** — it is visually dragging the strings down (monster doesn't move, strings move)
- Contact with monster = normal hit (−6s) — it is a soft floor, not an instant kill
...
**Boundary:** Rectangle at screen center with a **solid floor and ceiling** (visible walls — heart physically cannot pass through)
...
| W or Space (hold) | Full height jump — height scales with hold duration up to a maximum cap |
| Jump count | Single jump only — no double jump |

**Projectile Types:** Arrow-type spikes

**Special Attack Modifiers:**
- **Platforms** — appear as landing surfaces in certain attack patterns
- **Gravity Flip** — gravity reverses in certain attacks; heart rotates to reflect new direction; floor becomes ceiling
...
**Indicators:** Faint telegraph lines → expand into lethal beams after 1.5s warning → beam fires for 0.5s
...
> ⚠️ Note: Track 1 constraint means slicing must use cursor SWIPE, not mouse clicks
...
**Timer Economy:** TBD — does missing a fruit cost time? Does slicing give time?
...
**Timer Economy:** TBD — does destroying blocks give +time?

---

### 9. KAMIKAZE HELL
> *Auto-scrolling breakout — planes, dashing*

**Heart Color:** TBD

**Boundary:** TBD — infinite auto-scroll to the right
...
### 10. DARK MAZE HELL
> *Reference: Jackenstein from Deltarune — navigate blind*
...
**Mechanics:** Screen-spanning walls approach rapidly. One randomized transparent gap per wall. Must align heart with the gap. Walls speed up logarithmically over 30s–1min runtime.

**Timer Economy:** TBD — does threading the gap closely give +time?
...
**Timer Economy:** TBD — does a mistake cost time? Does perfect sequence give +time?

---

### 13. MUSIC HELL (BOSS MODE)
> *Piano keyboard bottom, falling note blocks synced to music — most intense phase*
